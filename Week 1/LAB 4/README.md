...
1. Supported SSH key formats
    > Read the [Docs](https://docs.microsoft.com/en-us/azure/virtual-machines/linux/mac-create-ssh-keys)
2. Create an SSH key pair
    ...
    Enter file in which to save the key (/home/poly4/.ssh/id_rsa): tukeypair
...
3. Provide an SSH public key when deploying a VM
    ...
    * To Provide the SSH public key when deploying a VM, provide the `--ssh-key-values` where the public key files is
    ...
    ```
    az vm create  
    --resource-group MyResourcegroupdisk --name poly4VM --image UbuntuLTS --admin-username poly4 --ssh-key-values tukeypair.pub
    ```
...
4. SSH into your VM
    ...
    > Code input:
    ```
    ssh -i tukeypair poly4@20.124.99.169
    ```
